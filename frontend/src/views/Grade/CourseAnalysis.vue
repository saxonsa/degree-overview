...
import { getResultAnalysis } from '@/api/grade.js'
...
export default {
  ...
  created () {
    ...
    this.breadItems[1].text = 'Analysis result on ' + this.course.code + ' ' + this.course.name
  ...
  methods: {
    ...
    generateAnalysisChart () {
      this.ciloChart.showLoading({
        text: 'Loading analysis data on CILO...',
        effect: 'whirling'
      })
      getResultAnalysis({
        ...
        const CILOLength = this.ciloData.length
        var CILOArr = []
        for (var c = 0; c < CILOLength; c++) {
          CILOArr.push('CILO' + (c + 1).toString())
        }

        // legend
        var legend = []
        for (var i = 0; i < res.result.length; i++) {
          legend.push(res.result[i].year)
        }
        ...
        for (i = 0; i < res.result.length; i++) {
          var gradeArr = []
          for (var j = 0; j < this.ciloData.length; j++) {
            gradeArr.push(res.result[i].cilo_score['index' + (j + 1)].toFixed(2))
          }

          var grade = {
            name: res.result[i].year,
            type: 'bar',
            data: gradeArr,
            markPoint: {
              data: [
