<template>
  <v-app id="inspire">
    <v-app-bar
      ...
      min-height="20"
    >
      <a class="dashboard-nav-brand-title">DegreeOverview</a>
          ...
          >
            ...
            >
              ...
              >
                ...
                <v-btn v-if="me.role!='student'" slot="append" tile style="left: 12px;" @click="searchCourse">
              ...
              <a-spin tip="Loading" :spinning="courseListSpin">
                  ...
                  >
                    ...
                    <v-expansion-panel-content style="margin-top: 20px; margin-left: 12px;">
                      ...
                      <v-btn
                        ...
                        class="lower-letter-btn white--text"
                        color="blue darken-3"
                        @click="gotoStudentPerformance(item.course_code, item.course_name)"
                      >Course Performance
                      </v-btn>
                      <v-btn
                        v-else
                        ...
                        class="lower-letter-btn white--text"
                        color="blue darken-3"
                        @click="gotoCourseAnalysis(item.course_code, item.course_name)"
                      >Analysis Results
                      </v-btn>
                      <v-btn
                        small
                ...
                <v-alert
                  v-if="resultType == 'by course (with code)'"
                  border="left"
                  color="indigo"
                  dark
                >
                  Below are the prerequisite courses of
                  {{ courses[0].course_code }} ({{ courses[0].course_type }}) {{ courses[0].course_name }}
                </v-alert>
                ...
                >
                  <v-expansion-panel
                    v-for="(item, key) in courses[0].prerequisite_course"
                    :key="key"
                  >
                    <v-expansion-panel-header>
                      <span style="font-size: 14px;">
                    ...
                    <v-expansion-panel-content>
                      ...
                      <!-- 在by course (with code) 搜索中阉割了 prerequisite 课程中的 prerequisite信息 -->
                      <!-- <v-divider v-if="item.prerequisite_course[0]"></v-divider>
                      <v-row v-if="item.prerequisite_course[0]">
                        <span style="margin-top: 15px; margin-left: 14px; margin-bottom: 20px;"> Prerequisite Course:  </span>
                        <br/>
...
import { searchCourseByTypeAndKeyword, displayAllCourse, getStudentCourseList } from '@/api/system.js'
...
export default {
  ...
  created () {
    if (this.me.role === 'student') {
      if (!this.alertMsg) {
        this.alertMsg = 'Below are the courses you have taken'
        this.alert = true
      }
    ...
    document.onkeydown = function (e) {
      ...
      if (that.$route.path === '/' && (e.code === 'Enter' || e.code === 'enter')) {
        if (that.me.role === 'student') {
          that.searchStudentCourse()
        } else {
          that.searchCourse()
        }
      }
    ...
    searchStudentCourse: function () {
      ...
      if (this.searchValue && this.searchType) {
        ...
        getStudentCourseList({
          ...
          this.courses = res.result.courseList
          this.total_page = res.result.total_page
          if (!this.courses[0]) {
            this.alertMsg = 'No courses found'
            this.alert = true
          }
          this.courseListSpin = false
        }).catch(error => {
